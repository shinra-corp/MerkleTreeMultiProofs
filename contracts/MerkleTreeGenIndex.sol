...
library MerkleTreeGenIndex {
    ...
    bytes32 constant ZERO = keccak256(abi.encodePacked(uint256(0)));
    ...
    returns(bytes32)
    {
        ...
        //Loop leafs until find ZERO or complete the array
        ...
        while(index < giop.length) {
            ...
            if(giop[giop_pointer] % 2 == 0) {
                ...
                self.ram[giop[parentId]] = keccak256(abi.encodePacked(self.ram[giop[giop_pointer] - 1], nodes[index]));

            } else {
                self.ram[giop[parentId]] = keccak256(abi.encodePacked(nodes[index], self.ram[giop[giop_pointer] + 1]));
            }
            index += 1;
        ...
        self.ram[parentId] = keccak256(abi.encodePacked(self.ram[2*parentId + 1], self.ram[2*parentId + 2] ));
